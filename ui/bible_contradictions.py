"""
bible_contradictions.py — Onglet Contradictions de la Bible

Affiche les contradictions détectées par l'Archiviste (fond rouge discret).
"""
...
QTableWidget::item:selected {{ background-color: #6b1a1a; color: #ffffff; }}
QHeaderView::section {{
...
_ROW_BG = QColor("#2a1515")   # fond rouge très sombre pour chaque ligne


class BibleContradictionsWidget(QWidget):
    """Onglet Contradictions — tableau titre / explication."""
    ...
    def _setup_ui(self):
        ...
        self._empty_hint = QLabel("Aucune contradiction détectée.\nL'Archiviste surveille les incohérences automatiquement.")
        ...
        self._table.setHorizontalHeaderLabels(["Contradiction", "Explication", ""])
    ...
    def refresh(self, bible_db: BibleDB) -> None:
        ...
        rows = bible_db.get_all("contradictions")
        if not rows:
            self._table.hide()
        ...
        for i, r in enumerate(rows):
            item_label   = QTableWidgetItem(r["label"])
            item_content = QTableWidgetItem(r["content"])
            item_label.setBackground(_ROW_BG)
            item_content.setBackground(_ROW_BG)
            self._table.setItem(i, 0, item_label)
            self._table.setItem(i, 1, item_content)
    ...
    def _delete(self, label: str) -> None:
        ...
        if reply == QMessageBox.StandardButton.Yes:
            self._bible_db.delete("contradictions", label)
