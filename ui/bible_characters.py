"""
bible_characters.py — Onglet Personnages de la Bible

Affiche la liste des personnages extraits par l'Archiviste.
Lecture seule (l'Archiviste alimente, l'auteur consulte).

Refresh via refresh(bible_db) — appelé par BiblePanel quand la Bible est mise à jour.
"""
...
class BibleCharactersWidget(QWidget):
    """Onglet Personnages — tableau label / description."""
    ...
    def _setup_ui(self):
        ...
        self._table.setHorizontalHeaderLabels(["Personnage", "Description", ""])
    ...
    def refresh(self, bible_db: BibleDB) -> None:
        """Recharge depuis la base. Appeler apres chaque bible_updated."""
        self._bible_db = bible_db
        rows = bible_db.get_all("characters")
        if not rows:
            self._table.hide()
    ...
    def _delete(self, label: str) -> None:
        ...
        if reply == QMessageBox.StandardButton.Yes:
            self._bible_db.delete("characters", label)
